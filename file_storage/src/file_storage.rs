...
impl <T: Deserializable> FileStorage<T> {
    ...
    pub fn search_by_key(&mut self, search_value: impl PartialEq + fmt::Display, key_pos: u32) -> Option<T> where T: Deserializable{
        let mut buffer = String::new();
        ...
        while let Some(mut line) = self.read_line(&mut buffer, reset) {
            ...
            for i in 0..key_pos {
                attribute = self.read_attribute(&mut line).unwrap().chars().filter(|&c| c != ' ' && c != '\n').collect::<String>();
            }
            if attribute == search_value.to_string() {
                let mut buffer = String::new();
                self.cursor -= line_start as u64;
